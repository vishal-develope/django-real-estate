...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(self, username, first_name, last_name, **extra_fields):
        if not username:
            raise ValidationError(_('User must submit user name'))
        
        if not first_name:
            raise ValidationError(_('User must submit first name'))
        
        if not last_name:
            raise ValidationError(_('User must submit last name'))
